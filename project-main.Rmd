...
*Throughout our analysis, we recognise that the Lifestyle factor reflects important long-term behavioural influences, yet we exclude it from the main two-factor specification since its impact differs markedly across regions—improving model performance in lifestyle-driven areas like the Central Coast but diminishing it in neutral regions such as Blacktown. Accordingly, it is retained solely for comparison and illustrative discussion rather than as a key explanatory variable.*
...
dof_lookup <- tibble(
  model_name = c("autoarima", "rw_drift", "arima211", "arima110", "arima111", "sarima211", "piece", "step"),
  dof = c(NA_integer_, 0, 3, 1, 2, 4, 2, 3)
)
...
kable(lb_tbl_u, caption = "Ljung-Box Test Results for Model Residual on Market factor") 
```
...
fit_PS <- train_PS |>
  model(
    arima201 = ARIMA(PS ~ boom + decline + pdq(2,0,1)), 
    arima100 = ARIMA(PS ~ boom + decline + pdq(1,0,0)),
    arima200 = ARIMA(PS ~ boom + decline + pdq(2,0,0)),
    arima101 = ARIMA(PS ~ boom + decline + pdq(1,0,1))
  ) |> 
  select(arima201)

glance(fit_PS) |> arrange(AICc) 
...
kable(lb_tbl_ps, caption = "Ljung-Box Test Results for Model Residual on Mining factor") 
```
...
# Add labels
acc_U  <- acc_U  |> mutate(series = "U (market)")
...
As a result, five ARIMAX models were selected and fitted across regions according to their individual time series dynamics. Among the 88 SA4 regions, 75 passed the Ljung–Box test, indicating that their residuals approximate white noise.
...
results <- df_all |>
  ...
  reframe({
    dates <- month_date
    y <- value
    X <- as.matrix(pick(market, mining_ps, lifestyle))
   ...
   # placeholders 
    ...
    b <- coef(best_fit)[c("market", "mining_ps", "lifestyle")]
    tibble(
      best_model = paste0("ARIMAX(", best, ") d=0"),
      AICc = best_AICc,
      LB_p12 = best_p, #LB at lag 12 
      market = unname(b[1]),
      mining_ps = unname(b[2]),
...
reg_coefs_arimax <- results |> select(region, market, mining = mining_ps, lifestyle)
```

```{r reg-arimax, eval = FALSE}
# Baseline fit 
fit_xreg <- df_all |>
  group_by(region) |>
  group_map(~{
...
# arimax 
var_eps_arimax <- results |>
  transmute(region, var_eps = map_dbl(model, \(m) m$sigma2))
