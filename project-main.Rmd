...
# Change coefficient into PS spread 
sa4_indexes_long <- sa4_indexes |>
  pivot_longer(cols = -month_date, names_to = "region", values_to = "value")

reg_coefs_miningps <- sa4_indexes_long |> 
  inner_join(factor_trends, by = "month_date") 

reg_coefs_ps <- reg_coefs_miningps |> 
  group_by(region) |> 
  group_modify(~{
    fit <- lm(value ~ market + mining_ps + lifestyle, data = .x)
    gl  <- glance(fit)
    tt  <- tidy(fit)
    dw  <- tryCatch(dwtest(fit)$statistic[[1]], error = function(e) NA_real_)
    tibble(
      alpha = tt$estimate[tt$term == "(Intercept)"],
      market = tt$estimate[tt$term == "market"],
      mining = tt$estimate[tt$term == "mining_ps"],
      lifestyle = tt$estimate[tt$term == "lifestyle"],
      r2 = gl$r.squared %||% NA_real_,
      durbin_watson = dw)}) |> 
  ungroup() |> 
  select(region, alpha, r2, durbin_watson, market, mining, lifestyle)
...
sa4_raw <- sa4_indexes |>
  select(
    ...
    `PERTH - SOUTH EAST`, `PERTH - NORTH WEST`, 
    ...
    `RIVERINA`, `CENTRAL WEST`, `MORETON BAY - NORTH`,
    `CAPITAL REGION`, `LAUNCESTON AND NORTH EAST`, `COFFS HARBOUR - GRAFTON`
...
lifestyle_cmp |> 
  ...
  geom_line() +
  labs(x = "Period", y = "Standardised value", colour = "Series",
       title = "Reconstructing PC3 Using SA4 Region Combinations") +
  theme_minimal()

corr_lifestyle <- cor(lifestyle_cmp$pc_value, lifestyle_cmp$lifestyle_proxy, use = "complete.obs") |> print()
```
...
fit_U |> 
  select(arima211) |>
  gg_tsresiduals() + 
  ggtitle("Residual Diagnostics for ARIMA(2,1,1)",
          subtitle = "Model fit on U series")
```
...
fit_PS |> 
  ...
  ggtitle("Residual Diagnostics for ARIMA(2,0,1)",
          subtitle = "Model fit on δPS series")
```
...
fc_PS <- fabletools::forecast(fit_PS, new_data = future_PS , h = h, level = c(80, 95))
...
autoplot(fc_U, factor_trends_ts) +
  ...
  geom_text(data = last_row_U,
            aes(x = date, y = hi80,
                ...
                colour = .model),
            vjust = -0.6, size = 2.5) + 
  labs(title = "Forecasts of National Trend U with 80% Prediction Interval",
       subtitle = "The forecasts show a rising trend with wider prediction intervals")
...
autoplot(fc_PS, factor_trends_ts) +
  ...
  geom_text(data = last_row_PS,
            aes(x = date, y = hi80,
                ...
                colour = .model),
            vjust = -0.6, size = 2.5) + 
  labs(title = "Forecasts of Mining Trend δPS with 80% Prediction Interval",
...
df_fc_U  <- as_tibble(fcbands_U)
df_fc_PS <- as_tibble(fcbands_PS)

df_fc <- df_fc_U |>
  select(date, mean_U = mean, lo80_U = lo80, hi80_U = hi80,
         lo95_U = lo95, hi95_U = hi95) |>
  left_join(
    df_fc_PS |>
      select(date, mean_PS = mean, lo80_PS = lo80, hi80_PS = hi80,
         lo95_PS = lo95, hi95_PS = hi95),
    by = "date"
  )
...
The ARIMAX approach retains fixed regression coefficients while modelling the residuals with an ARIMA process. Several baseline specifications (1,1,0),(0,1,1),(0,1,0),(1,1,1) were tested, and the optimal model was chosen by balancing AICc performance with Ljung–Box test results, ensuring that the residuals approximated white noise.

The ARIMA(1,1,0) specification achieved the strongest overall performance, with 69.3% of SA4 regions exhibiting residuals whose Ljung–Box p-values exceeded the 5% threshold, indicating adequately whitened residuals. 

For the remaining regions that did not meet this criterion, we analysed their residual patterns to identify the underlying causes of autocorrelation. Regions displaying clear seasonal patterns were refitted using a seasonal ARIMA(1,0,0)[12]model, while those showing short-lag dependence were assigned an AR(2) structure. Incorporating these targeted refinements raised the proportion of regions passing the diagnostic test to 86.4%, substantially improving overall model adequacy.

(need to add ARIMAX models explanation here)

```{r reg-arimax}
# Define variables 
x_df <- factor_trends |> select(month_date, market, mining_ps, lifestyle)

df_all <- sa4_indexes_long |>
...
fit_xreg <- df_all |>
  ...
  group_map(~{
    ...
    y <- .x$value
    reg <- .y$region[[1]]  
    x <- as.matrix(.x[, c("market","mining_ps","lifestyle")])
    y_ts <- ts(y, frequency = 12, start = c(year(min(dates)), month(min(dates))))
    fit <- Arima(y_ts, order = c(1, 1,0), xreg = x, include.mean = FALSE, include.drift = FALSE) 
    resid <- residuals(fit)
    lb <- Box.test(resid, lag = 24, type = "Ljung-Box", fitdf = length(coef(fit)))
    tibble(region = reg,
           actual = y,
           y_hat = as.numeric(fitted(fit)),
           model = list(fit),
           resid = list(resid),
           aic = AIC(fit),
           lb_p = lb$p.value) }) |> 
...
# Regions for finer fit 
region_not_fit <- fit_xreg |> 
  filter(lb_p < 0.05) |> 
  select(region)|> 
  unique() |>
  pull(region)

short_lag_regions <- c("BAROSSA - YORKE - MID NORTH","BENDIGO","BRISBANE - SOUTH","CAPITAL REGION","GEELONG","LAUNCESTON AND NORTH EAST","MACKAY - ISAAC - WHITSUNDAY","MELBOURNE - NORTH WEST","MORETON BAY - NORTH","NEWCASTLE AND LAKE MACQUARIE")

seasonal_regions <- c("BALLARAT","BUNBURY","CENTRAL WEST","DARWIN","HOBART", "ILLAWARRA","IPSWICH","MELBOURNE - SOUTH EAST","PERTH - NORTH WEST","PERTH - SOUTH WEST","SOUTH EAST","SYDNEY - BAULKHAM HILLS AND HAWKESBURY","SYDNEY - INNER SOUTH WEST","SYDNEY - OUTER WEST AND BLUE MOUNTAINS","WESTERN AUSTRALIA - OUTBACK (NORTH)","WIDE BAY")

# Finer fit - two models 
fit_xreg_seasonal <- df_all |>
  filter(region %in% seasonal_regions) |>
  group_by(region) |>
  group_map(~{
    dates <- .x$month_date
    y <- .x$value
    ...
    x <- as.matrix(.x[, c("market","mining_ps","lifestyle")])
    y_ts <- ts(y, frequency = 12, start = c(year(min(dates)), month(min(dates))))
    fit <- Arima(y_ts, order = c(1,1,0), seasonal = list(order = c(1,0,0), period = 12),xreg = x, include.mean = FALSE, include.drift = FALSE)
    resid <- residuals(fit)
    lb <- Box.test(resid, lag = 24, type = "Ljung-Box", fitdf = length(coef(fit)))
    tibble(
      region = reg,
      spec_label = "seasonal (1,1,0)(1,0,0)[12]",
      model = list(fit),
      resid = list(resid),
      aic = AIC(fit),
      lb_p = lb$p.value
...
fit_xreg_short <- df_all |>
  filter(region %in% short_lag_regions) |>
  group_by(region) |>
  group_map(~{
    dates <- .x$month_date
    y <- .x$value
    ...
    x <- as.matrix(.x[, c("market","mining_ps","lifestyle")])
    y_ts <- ts(y, frequency = 12, start = c(year(min(dates)), month(min(dates))))
    fit <- Arima(y_ts, order = c(2,1,0), xreg = x, include.mean = FALSE, include.drift = FALSE) 
    resid <- residuals(fit)
    lb <- Box.test(resid, lag = 24, type = "Ljung-Box", fitdf = length(coef(fit)))
    tibble(
      region = reg,
      spec_label = "short lag (2,1,0)",
      model = list(fit),
      resid = list(resid),
      aic = AIC(fit),
      lb_p = lb$p.value)}) |>
...
# Binding model testing outcomes
results_arimax <- bind_rows(
  fit_xreg |> 
    summarise(pass = mean(lb_p > 0.05, na.rm = TRUE),
              mean_AIC = mean(aic, na.rm = TRUE)) |> 
    mutate(model = "Baseline ARIMA(1,1,0)"),
  
  fit_xreg_short |> 
    summarise(pass = mean(lb_p > 0.05, na.rm = TRUE),
              mean_AIC = mean(aic, na.rm = TRUE)) |> 
    mutate(model = "Short lag ARIMA(2,1,0)"),
  
  fit_xreg_seasonal |> 
    summarise(pass = mean(lb_p > 0.05, na.rm = TRUE),
              mean_AIC = mean(aic, na.rm = TRUE)) |> 
    mutate(model = "Seasonal ARIMA(1,1,0)(1,0,0)[12]")
)

kable(results_arimax, caption = "Model Diagnostics: p-values and AICc across ARIMA Variants")

# Bind all airmax models 
refit_regions <- c(short_lag_regions, seasonal_regions)
fit_xreg_clean <- fit_xreg |> filter(!region %in% refit_regions)

fit_all <- bind_rows(
  fit_xreg_clean |> mutate(spec_label = "baseline (1,1,0)"),
  fit_xreg_short,
  fit_xreg_seasonal
) 

reg_coefs_arimax <- fit_all |>
  group_by(region, spec_label) |> 
  slice(1) |> 
  ungroup() |> 
  mutate(coefs = map(model, coef)) |>
  unnest_wider(coefs) |> 
  select(region, market, mining = mining_ps, lifestyle, spec_label)

# This is the code for the PCF, PACF plots for the regions with p under 0.05 
# for (r in region_not_fit ) {
#  resid_list <- fit_xreg |> filter(region == r) |> pull(resid)
 # if (length(resid_list) == 0) next
#  resid_vals <- resid_list[[1]]

 # p1 <- ggAcf(resid_vals)  + ggtitle(paste("Residual ACF -", r))
#  p2 <- ggPacf(resid_vals) + ggtitle(paste("Residual PACF -", r))
#  gridExtra::grid.arrange(p1, p2, ncol = 2)}
```

This section compares the coefficient distributions for three factors across two modelling approaches: OLS and ARIMAX. The OLS model (blue) represents a static regression using contemporaneous factors only, while the ARIMAX model (red) incorporates time-series autocorrelation to account for temporal dependencies in the residuals.

- Market Factor: Both models yield strong positive coefficients with tightly clustered distributions. The medians are nearly identical, and the IQRs are narrow. This consistency confirms that national market trends exert a robust and uniform influence on regional price movements, irrespective of the modelling framework.

- Mining Factor: Median coefficients are close to zero in both models. However, the OLS model exhibits a wider spread, including more positive outliers. In contrast, ARIMAX produces smaller and more stable estimates. This suggests that once time-series persistence is accounted for, the mining signal is partially absorbed, reducing its apparent impact while retaining its relevance.

- Lifestyle Factor: Both models show similar distributions, with medians around 0.3 and comparable IQRs. ARIMAX coefficients are more tightly clustered, indicating a slight attenuation of the lifestyle signal when autocorrelation is modelled. This implies that lifestyle effects are modest and less regionally variable once temporal dynamics are considered.

```{r coef-compare}
# Compare coefficients: OLS & ARIMAX 
reg_coefs_ps <- reg_coefs_ps |> mutate(model_type = "OLS")
reg_coefs_arimax <- reg_coefs_arimax |> mutate(model_type = "ARIMAX")

reg_compare <- bind_rows(reg_coefs_ps, reg_coefs_arimax) |> 
...
reg_compare |>
  ...
  labs(
    title = "Coefficient Comparison: OLS (PS) vs ARIMAX",
    x = NULL,
    y = "Coefficient Value"
  ) +
  theme_minimal() +
...
```{r roll-ridge, eval = FALSE}
# Config set up cv for lambda 
target <- "value"
predictors <- c("market","mining_ps","lifestyle")
win <- 60 
...
panel <- sa4_indexes_long |>
  inner_join(factor_trends, by = "month_date") |> 
  select(-mining) |> 
  mutate(across(c(market, mining_ps, lifestyle),  ~ as.numeric(scale(.)))) |> # Scale predictors 
  arrange(region, month_date) |> 
  drop_na(all_of(c(target, predictors)))

# Pull everything together in a matrix 
X_all <- as.matrix(panel[, predictors, drop = FALSE])
y_all <- panel[[target]]

# Cross-validation to choose lambda
cvfit <- cv.glmnet(x = X_all, y = y_all, alpha = 0, family = "gaussian", intercept = TRUE, standardize = FALSE, nfolds = 5)
...
# List and index for storing results
all_rows <- list()
row_counter <- 1L

# Main function 
for (r in regions) {
  df_r <- panel[panel$region == r, , drop = FALSE]
  n_r <- nrow(df_r)

  for (i in seq.int(win, n_r)) {
    rows_win <- seq.int(i - win + 1L, i)
    dfw <- df_r[rows_win, , drop = FALSE]
    if (anyNA(dfw[, c(target, predictors), drop = FALSE])) next

    X <- as.matrix(dfw[, predictors, drop = FALSE])
    y <- dfw[[target]]
    ...
    a0 <- as.numeric(C["(Intercept)", 1])
    betas <- numeric(length(predictors))
    names(betas) <- predictors
    
    for (p in predictors) {
      betas[p] <- if (p %in% rownames(C)) as.numeric(C[p, 1]) else 0
    }
    t_end <- dfw$month_date[win]

    all_rows[[row_counter]] <- tibble(
      region = r,
      month_date = t_end,
      alpha = a0,
      !!!as.list(betas)
...
# Result 
coefs_ridge_roll <- bind_rows(all_rows)
write_csv(coefs_ridge_roll, "data/df_ridge_coef.csv")

#names_unique <- unique(coefs_ridge_roll$region) |> print()

# Plot the rolling coefficients for selected regions
s_mel <- coefs_ridge_roll |>
  filter(region == "MELBOURNE - INNER") |>
  arrange(month_date) |>
  pivot_longer(all_of(predictors), names_to = "factor", values_to = "beta")

s_syd <- coefs_ridge_roll |>
  filter(region == "SYDNEY - CITY AND INNER SOUTH") |>
  arrange(month_date) |>
  pivot_longer(all_of(predictors), names_to = "factor", values_to = "beta")

# !!double check this plot 
ggplot(s_mel, aes(x = month_date, y = beta, colour = factor)) +
  geom_line() +
  geom_smooth(se = FALSE, method = "loess", span = 0.2, linewidth = 0.6, alpha = 0.6) +
  labs(
    title = "Rolling coefficients (Melbourne)", x = NULL, y = "Coefficient", colour = "Predictor") +
  theme_minimal(base_size = 12)

# Distribution of rolling coefficients for all regions
all_long <- coefs_ridge_roll |> 
  pivot_longer(all_of(predictors), names_to = "factor", values_to = "beta")

ggplot(all_long, aes(x = beta)) +
  geom_density() +
  facet_wrap(~ factor, scales = "free_y") +
  labs(
    title = "Distribution of rolling coefficients (all regions)", x = "Coefficient", y = "Density") +
  theme_minimal()
```

The expanding window approach was used to assess the stability of model coefficients over time. By gradually extending the estimation sample, we tested whether the relationships between regional housing indices and key factors remained consistent as new data of the following years was added. We refitted the OLS regression twice: First using data up to June 2021, and again including data up to June 2023. Comparing these two sets of coefficients allowed us to evaluate how much the estimated sensitivities of each factor changed when the sample expanded by two years.

The table results displays the average yearly change and standard deviation of coefficients across regions. The intercept and market factors show small but positive shifts, while mining and lifestyle remain nearly unchanged. This suggests that the overall influence of these factors on housing prices has been highly stable between 2021 and 2023. While whe scatter plot compares the 2021 and 2023 coefficients for each factor. Points closely follow the 45° line, confirming that the factor effects are nearly identical across years. 
...
start_date <- as.Date("1998-06-01")
end_date   <- as.Date("2023-06-01")
cutoffs    <- as.Date(c("2021-06-01", "2023-06-01")) 
...
coef_diff <- coefs_cuts |>
  ...
  rename(as_of_2021 = `2021-06-01`, as_of_2023 = `2023-06-01`) |>
  mutate(diff = as_of_2023 - as_of_2021) 
...
kable(coef_diff_tbl, 
      caption = "Mean and Standard Deviation of Coefficient Changes (2021-2023)", digits = 4)

ggplot(coef_diff, aes(x = as_of_2023, y = as_of_2021, colour = factor)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_point(alpha = 0.8) +
  facet_wrap(~factor, scales = "free") +
  labs(
    title = "Coefficient Stability: 2021 vs 2023",
    subtitle = "Coefficients show strong year-to-year stability across regions",
    x = "Coefficient (up to 2021-06)",
    y = "Coefficient (up to 2023-06)"
  ) +
  theme_minimal()
...
The analysis was conducted using coefficients derived from both the OLS and ARIMAX specifications. While the OLS model provides a static benchmark, the ARIMAX model extends it by accounting for temporal dependence and reducing residual autocorrelation. In both cases, the mining factor spread was treated as the only varying source of uncertainty, with all other factors held constant. For each region, the 10-year growth implied by the market factor was projected, and a 95 per cent multiplicative uncertainty band was computed based on the volatility of the mining factor and the residual variance from each model. 

```{r uncertainty-lv}
# Variance from 95% interval of PS 
df_fc_PS <- df_fc |>
  mutate(
    half_width_95 = (hi95_PS - lo95_PS) / 2,
    # Standard deviation 
    sd_PS_95 = half_width_95 / 1.96,
    # Variance
    var_PS_95 = sd_PS_95^2
  )

# Pull the variance 
sigma_delta_10yPS <- df_fc_PS |> 
  filter(date == max(date)) |> 
  pull(var_PS_95)

# Rerun regression using only market and mining on city index 
#city_indexes_long <- city_indexes |>
 # pivot_longer(cols = -month_date, names_to = "region", values_to = "mu_r")

reg_panel <- sa4_indexes_long |>
  left_join(factor_trends |> select(month_date, U = market, PS = mining_ps), by = "month_date") 

reg_coefs2 <- reg_panel |>
  group_by(region) |>
  do({
    fit <- lm(value ~ U + PS, data = .)
    tidy_fit <- tidy(fit)
    glance_fit <- glance(fit)
    tibble(
      beta = tidy_fit$estimate[ tidy_fit$term == "U" ],
      lambda = tidy_fit$estimate[ tidy_fit$term == "PS" ],
      r2 = glance_fit$r.squared,
      var_eps = glance_fit$sigma^2
    )
  })

# 95% multiplicative uncertainty band - ols 
reg_uncertainty_ols <- reg_coefs2 |>
  mutate(
    sd_r = sqrt(lambda^2 * sigma_delta_10yPS + var_eps),
    band95_low = exp(-1.96 * sd_r),
    band95_high = exp(1.96 * sd_r),
    band_width = band95_high - band95_low,
    perc_error = (band95_high - 1) * 100
  )

# arimax 
var_eps_arimax <- fit_all |> 
  group_by(region) |> 
  summarise(var_eps = map_dbl(model, ~ .x$sigma2) |> mean(na.rm = TRUE), .groups = "drop")

coefs_arimax <- reg_coefs_arimax |> 
  transmute(region,
            beta = market,
            lambda = mining,
            gamma = lifestyle)

# 95% multiplicative uncertainty band - arimax
reg_uncertainty_arimax <- coefs_arimax %>%
  inner_join(var_eps_arimax, by = "region") %>%
  mutate(
    sd_r = sqrt(lambda^2 * sigma_delta_10yPS + var_eps),
    band95_low  = exp(-1.96 * sd_r),
    band95_high = exp( 1.96 * sd_r),
...
# Summary table 
reg_uncertainty_summary <- reg_uncertainty_arimax |>
  mutate(
    bracket = case_when(
      perc_error < 10 ~ "0–10%",
      perc_error < 20 ~ "10–20%",
  ...
  summarise(
    n_regions = n(),
    avg_error_pct = mean(perc_error, na.rm = TRUE),
    min_error_pct = min(perc_error, na.rm = TRUE),
    max_error_pct = max(perc_error, na.rm = TRUE)
  ) |>
  arrange(bracket)

kable(reg_uncertainty_summary,
      caption = "Distribution of 10-Year Market-Only Forecast Uncertainty (ARIMAX Model)",
      digits = 2)

# Plot comparison - OLS and arimax 
reg_uncertainty_wide <- reg_uncertainty_ols |>
  select(region, perc_error_ols = perc_error) |>
  inner_join(reg_uncertainty_arimax |> select(region, perc_error_arimax = perc_error), by = "region")

ggplot(reg_uncertainty_wide, aes(x = perc_error_ols, y = perc_error_arimax)) +
  geom_point(alpha = 0.7) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", colour = "red") +
  labs(
    title = "Region-wise Comparison of Forecast Uncertainty",
    subtitle = "Points below the dashed line mean ARIMAX produces narrower uncertainty bands",
    x = "OLS Percentage Error",
    y = "ARIMAX Percentage Error"
  ) +
  theme_minimal()
```


# Appendix: Long-term projection and macroeconomic scenarios
