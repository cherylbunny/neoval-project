...
**WIP - Notes**: Before complicating with rolling betas, we are examining if ARIMAX can soak up the persistence. If residual autocorrelation disappears after ARIMAX, then maybe static coefficients are fine. 
...
results_no_const <- df_all |>
  ...
  group_map(~{
    ...
    # Fit ARIMA(1,1,0) with xreg
    fit   <- Arima(y_ts, order = c(1,1,0), xreg = x, include.mean = FALSE, include.drift = FALSE) # No intercept or drift
    resid <- residuals(fit)
...
# manual try: 010 0.3863636, 110 0.6931818, 011 0.6704545, 111 0.6818182
...
ggplot(results_no_const, aes(lb_p)) +
  geom_histogram(bins = 20) +
  geom_vline(xintercept = 0.05, linetype = "dashed") +
  labs(title = "Residual Ljung–Box p-values across regions (ARIMAX(1,1,0))",
       x = "p-value", y = "Count",
       subtitle = "Proportion with p > 0.05: 69.3%") +
  theme_minimal()
```

# Model Improvement with Time-dependent Coefficients 

Forecasts for each major city and SA4 region are obtained by combining the projected factor trends with the static coefficients from the factor model. Specifically, the estimated loadings are applied to the forecasted values of U and PS to produce region-specific housing price index projections.
...
```{r ols-coef-wip, eval = FALSE}
panel <- sa4_indexes_long |>
  inner_join(factor_trends, by = "month_date") |> 
  select(-mining) |> 
  mutate(across(c(market, mining_ps, lifestyle), scale)) |> 
  arrange(region, month_date)

formula_y <- y ~ market + mining_ps

# Window setting  
win <- 120
predictors <- c("market","mining_ps") 
lambda <- 0.1  

empty_coef <- function() {
  tibble(alpha = NA_real_) |>
    bind_cols(as_tibble(setNames(
      as.list(rep(NA_real_, length(predictors))), predictors)))
}
...
coefs_ridge_roll <- panel |>
  ...
  mutate(.fits = slide_index(
    .x = pick(all_of(c("y", predictors))),
    .i = month_date,
    .before = win - 1,
    .complete = TRUE,
    .f = ~ {
      if (nrow(.x) < length(predictors) + 2) return(empty_coef())
      X <- as.matrix(.x[, predictors, drop = FALSE])
      yv <- .x$y
      fit <- glmnet(X, yv, alpha = 0, lambda = lambda,
...
coefs_tv |>
  filter(region == "PERTH - INNER") |>
  ggplot(aes(x = month_date)) +
  geom_line(aes(y = market, colour = "Market")) +
  geom_line(aes(y = mining_ps, colour = "Mining")) +
  labs(title = "Rolling coefficients for Inner Perth",
