...
```{r fitting-u-model-residual-test}
...
We experimented with several models for this series. First, we used a robust STL decomposition to separate the seasonal component from the low-frequency movement, then modelled the seasonally adjusted series with a simple ARIMA while allowing seasonality to be captured by a seasonal naïve. Introducing a single difference in the short-memory component produced a stable fit and whitened residuals. 

Since earlier analysis suggested the series could be rendered stationary by accounting for structural breaks, we explored two additional approaches. The first was an ARFIMA model, where deterministic seasonality and a known 2011 level shift were removed before estimating the fractional differencing parameter d. With `d < 0.5`, this indicated that full first-differencing may not be required. Finally, we de-seasonalised with STL, identified level breaks, introduced six step dummies at the detected dates, and captured the remaining short-memory with an ARMA specification on the regression residuals, which satisfied the Ljung–Box test for whiteness.

```{r fitting-ps-model}
# 1. STL can only fit random walk with drift otherwise ARMA term reports null model 
fit_PS1 <- train_PS |>
  model(
    stl = decomposition_model(
      STL(PS ~ season(window = "periodic") + # s window fixed seasonal shape
               trend(window = 13), # t window: annual smoothing, mean-reverting dynamic while not over-smoothing
          robust = TRUE),
      ARIMA(season_adjust ~ pdq(1,1,1)), 
      SNAIVE(season_year) # seasonal component model
    )
  )

fit_PS1 |> gg_tsresiduals()
augment(fit_PS1) |> features(.innov, ljung_box, lag = 24, dof = 3) #P value pass the test 

# 2. ARFIMA to determine differencing 
base <- train_PS |>
  mutate(step_2011 = as.integer(date >= yearmonth("2011 Apr")))
fit_PS2 <- base |>
  model(tslm = TSLM(PS ~ season() + step_2011))

fit_PS2_aug <- augment(fit_PS2)
fit_PS2_resid  <- fit_PS2_aug$.resid
y_ts <- ts(fit_PS2_resid, start = c(year(base$date[1]), month(base$date[1])), frequency = 12)

fit_PS2 <- fracdiff::fracdiff(y_ts, nar = 1, nma = 1)      # ARFIMA(1,d,1)
#fit_PS2$d   
Box.test(residuals(fit_PS2), lag = 24, type = "Ljung")  #P value pass the test 

# 3. Add more breaks and feed MA(1) on residuals. 
y_ts <- ts(train_PS$PS,
           start = c(year(train_PS$date[1]), month(train_PS$date[1])),
           frequency = 12)

# STL de-seasonalise
stl_fit <- stats::stl(y_ts, s.window = "periodic", robust = TRUE)
sa <- seasadj(stl_fit)  # seasonally adjusted

# breaks in level of de-seasonalised series
bp <- strucchange::breakpoints(sa ~ 1, h = 12)  
#zoo::as.yearmon(strucchange::breakdates(bp, breaks = bp$breaks.opt))

fit_PS3 <- train_PS |>
  mutate(
    step2003 = as.integer(date >= yearmonth("2003 Jun")),
    step2005 = as.integer(date >= yearmonth("2005 Feb")),
    step2007 = as.integer(date >= yearmonth("2007 Feb")),
    step2010 = as.integer(date >= yearmonth("2010 Jul")),
    step2013 = as.integer(date >= yearmonth("2013 Jun")),
    step2015 = as.integer(date >= yearmonth("2015 Feb"))
  ) |>
  model(tslm = TSLM(PS ~ season() + step2003 + step2005 + step2007 +
                                step2010 + step2013 + step2015))

det_aug <- augment(fit_PS3)
resid_ts <- ts(det_aug$.resid,
               start = c(year(min(train_PS$date)), month(min(train_PS$date))),
               frequency = 12)

m_ma1 <- Arima(resid_ts, order = c(1,0,1))  

Box.test(residuals(m_ma1), lag = 24, type = "Ljung") 
```

## Forecast Performance 

(WIP - on the PS fan width is too wide - will re-examine the model)
...
fc_PS <- forecast::forecast(fit_PS1, h = h, level = c(80, 95))
