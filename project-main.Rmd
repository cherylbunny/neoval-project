...
fit_xreg |>
  summarise(
    pass = mean(lb_p > 0.05, na.rm = TRUE),
...
```{r forecast-coef}
reg_coefs1 <- reg_coefs |> select(region_level, region, alpha, market, mining)

regional_forecasts <- df_fc |>
  crossing(reg_coefs1) |> 
  mutate(
    mu_fct = alpha + market * mean_U + mining * mean_PS,
    lo80 = alpha + market * lo80_U + mining * lo80_PS,
    hi80 = alpha + market * hi80_U + mining * hi80_PS,
    lo95 = alpha + market * lo95_U + mining * lo95_PS,
    hi95 = alpha + market * hi95_U + mining * hi95_PS
  ) |>
  select(date, region_level, region, mu_fct, lo80, hi80, lo95, hi95)

# Plot major city forecasted indexes 
major_city_fc <- regional_forecasts |>
  filter(region_level == "major_city")

ggplot(major_city_fc, aes(x = date, y = mu_fct, colour = region, fill = region)) +
  geom_line(size = 0.5) +
  labs(title = "Forecasted Housing Indexes for Major Cities",
       subtitle = "Trend differences between cities driven by their factor coefficient",
       x = "Date", y = "Forecasted Index",
...
We set a 60-month window to captures regime shifts faster and used grid search to identify the global λ that minimises out-of-sample RMSE across all regions. This λ is then applied in a rolling regression framework to estimate time-varying regional coefficients. The optimal λ, 0.0001, indicates a light penalty that nonetheless helps stabilise the estimates.

After that we fit rolling ridge regressions for each region using the chosen λ. The resulting time-varying coefficients capture evolving regional sensitivities to the national, mining and lifestyle trends, allowing for more nuanced forecasts that reflect changing market dynamics.

```{r ols-lambda, eval = FALSE}
# Set up panel 
...
# Function for empty coef when window is too short
empty_coef <- function(predictors) {
  tibble(alpha = NA_real_) |>
    bind_cols(as_tibble(setNames(as.list(rep(NA_real_, length(predictors))), predictors)))
}

# Config set up with grid search for lambda 
target <- "value"
predictors <- c("market","mining_ps","lifestyle")
win <- 60 
lambda_grid <- exp(seq(log(1e-4), log(1), length.out = 20)) # A grid of 20 even spaced values from 0.0001 to 1 

# Function to fit on first test window of rows, produce one coef 
ridge_predict_one <- function(train_df, test_row, predictors, target, lambda) {
  Xtr <- as.matrix(train_df[, predictors, drop = FALSE])
  ytr <- train_df[[target]]
  fit <- glmnet(x = Xtr, y = ytr, alpha = 0, lambda = lambda,
                intercept = TRUE, standardize = FALSE, family = "gaussian")
  x_te <- c(1, as.numeric(test_row[, predictors, drop = FALSE]))
  as.numeric(x_te %*% as.matrix(coef(fit)))
}

# Find the best global lambda 
rmse_by_lambda_global <- function(panel, predictors, target, win, lambda_grid) {
  
  errs_list <- panel |> 
    group_by(region) |> 
    group_map(~{
      df <- drop_na(.x, all_of(c(target, predictors))) #Drop NA is any, safe guard 
      n  <- nrow(df) 
      if (n < win) return(NULL) # Drop if not enough rows, safe guard 
      ends <- seq(win, n)
      
      map(lambda_grid, function(lam) { # Mapping the lambda grid
        se <- map_dbl(ends, function(i) { # Collect a single SSE per window per region 
          tr_idx <- (i - win + 1):(i - 1)  # Training 
          te_idx <- i   # Testing 
          if (length(tr_idx) < length(predictors) + 2) return(NA_real_) # Training set should be larger than num of predictors + 2
          yhat <- ridge_predict_one(df[tr_idx, , drop = FALSE], # Fit on training rows 
                                    df[te_idx, , drop = FALSE], # Predict on test row 
                                    predictors, target, lambda = lam) # Pass predictor name, outcome name and lambda
          (df[[target]][te_idx] - yhat)^2
          }) # Compute the squared error
        se
        }) |> 
        set_names(paste0("lam_", seq_along(lambda_grid))) |>as_tibble() # list of SE vectors
    })
  
  err_mat <- bind_rows(errs_list)
  rmse <- err_mat |> 
    summarise(across(everything(), ~ sqrt(mean(.x, na.rm = TRUE)))) |> # Compute RMSE
    pivot_longer(everything(), names_to = "lam_id", values_to = "rmse") |> 
    mutate(lambda = lambda_grid[as.integer(gsub("lam_", "", lam_id))]) |> 
    arrange(rmse) |> 
    select(lambda, rmse)
  rmse
}

# Lambda value by rmse, pick the first one with lowest RMSE 
rmse_table <- rmse_by_lambda_global(panel, predictors, target, win, lambda_grid)
lambda_best <- rmse_table$lambda[1]
```

```{r ols-roll}
# Fit the rolling OLS 
coefs_ridge_roll <- panel |> 
  group_by(region) |> 
  arrange(month_date, .by_group = TRUE) |> 
  mutate(
    .fits = slide_index( 
      .x = pick(all_of(c(target, predictors))),
      .i  = month_date,
      .before = win - 1,
      .complete = TRUE, # Only produce results when a full window exists
      .f = ~ {
        dfw <- drop_na(.x) # Remove any rows with missing values in target or predictors
        if (nrow(dfw) < length(predictors) + 2 || is.na(lambda_best)) {
          return(empty_coef(predictors))
        }
        X  <- as.matrix(dfw[, predictors, drop = FALSE]) # Predictor matrix
        yv <- dfw[[target]]
        fit <- glmnet(x = X, y = yv, alpha = 0, lambda = lambda_best, intercept = TRUE,
                      standardize = FALSE, family = "gaussian") # Fit ridge regression with chosen lambda 
        b <- as.numeric(coef(fit))
        tibble(alpha = b[1], !!!setNames(as.list(b[-1]), predictors))
      }
    )
  ) |> 
  ungroup() |> 
  select(region, month_date, .fits) |> 
  unnest(.fits)
```
